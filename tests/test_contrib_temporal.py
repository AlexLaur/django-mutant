...
class DateFieldDefinitionTest(
    TemporalFieldDefinitionTestMixin, BaseModelDefinitionTestCase
):
    field_definition_cls = DateFieldDefinition
    field_definition_init_kwargs = {"default": datetime.date(1990, 8, 31)}
    field_values = (datetime.date.today(), datetime.date(1988, 5, 15))


@override_settings(USE_TZ=False)
class NaiveDateTimeFieldDefinitionTest(
    TemporalFieldDefinitionTestMixin, BaseModelDefinitionTestCase
):
...
class AwareDateTimeFieldDefinitionTest(
    TemporalFieldDefinitionTestMixin, BaseModelDefinitionTestCase
):
    field_definition_cls = DateTimeFieldDefinition
    field_definition_init_kwargs = {
