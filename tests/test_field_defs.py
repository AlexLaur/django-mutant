...
class FieldDefinitionDeclarationTest(SimpleTestCase):
    def test_delete_override(self):
        ...
        self.assertTrue(
            any(msg in warning.message.args[0] for warning in catched_warnings)
        )
...
class FieldDefaultTest(BaseModelDefinitionTestCase):
    def test_clean(self):
        ...
        Model = self.model_def.model_class()
        self.assertEqual(
            Model.objects.create().field, module_level_pickable_default.incr
        )
    ...
    def test_create_with_default(self):
        ...
        IntegerFieldDefinition.objects.create_with_default(
            1337, name="field", model_def=self.model_def
        )
...
class FieldDefinitionChoiceTest(BaseModelDefinitionTestCase):
    def test_simple_choices(self):
        field_def = CharFieldDefinition.objects.create(
            name="gender", max_length=1, model_def=self.model_def
        )
        male_choice = FieldDefinitionChoice(
            field_def=field_def, value="Male", label="Male"
        )
        ...
        female_choice = FieldDefinitionChoice(
            field_def=field_def, value="F", label="Female"
        )
        ...
        choices = Model._meta.get_field("gender").get_choices(
            include_blank=False
        )
    ...
    def test_grouped_choices(self):
        field_def = CharFieldDefinition.objects.create(
            name="media", max_length=5, model_def=self.model_def
        )
        # Create Audio choices
        FieldDefinitionChoice.objects.create(
            field_def=field_def, group="Audio", value="vinyl", label="Vinyl"
        )
        FieldDefinitionChoice.objects.create(
            field_def=field_def, group="Audio", value="cd", label="CD"
        )
        # Create Video choices
        FieldDefinitionChoice.objects.create(
            field_def=field_def, group="Video", value="vhs", label="VHS Tape"
        )
        FieldDefinitionChoice.objects.create(
            field_def=field_def, group="Video", value="dvd", label="DVD"
        )
        # Create Unknown choices
        FieldDefinitionChoice.objects.create(
            field_def=field_def, value="unknown", label="Unknown"
        )
        # Make sure choices are correctly created
        Model = self.model_def.model_class()
        choices = Model._meta.get_field("media").get_choices(
            include_blank=False
        )
        expected_choices = [
            (
...
class FieldDefinitionManagerTest(BaseModelDefinitionTestCase):
    def test_natural_key(self):
        fd = CharFieldDefinition.objects.create(
            name="name", max_length=5, model_def=self.model_def
        )
