...
spatial_index_help_text = _(
    "Creates a spatial index for the given " "geometry field."
)
dim_help_text = _("Coordinate dimension.")
geography_help_text = _(
    "Creates a database column of type geography, " "rather than geometry."
)
...
class GeometryFieldDefinition(FieldDefinition):
    ...
    srid = models.IntegerField(
        _("SRID"), default=4326, help_text=srid_help_text
    )
    spatial_index = models.BooleanField(
        _("spatial index"), default=True, help_text=spatial_index_help_text
    )
    dim = models.PositiveSmallIntegerField(
        _("coordinate dimension"),
        choices=DIM_CHOICES,
        default=2,
        help_text=dim_help_text,
    )
    geography = models.BooleanField(
        _("geography"), default=False, help_text=geography_help_text
    )
