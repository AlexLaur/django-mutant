...
class FilePathFieldDefinition(CharFieldDefinition):
    path = DirectoryPathField(
        _("path"), max_length=100, help_text=path_help_text
    )
    match = RegExpStringField(
        _("match"),
        max_length=100,
        blank=True,
        null=True,
        help_text=match_help_text,
    )
    recursive = fields.BooleanField(
        _("recursive"), default=False, help_text=recursive_help_text
    )
