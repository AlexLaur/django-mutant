...
related_name_help_text = _(
    "The name to use for the relation from the "
...
class RelatedFieldDefinition(FieldDefinition):
    ...
    related_name = PythonIdentifierField(
        _("related name"),
        blank=True,
        null=True,
    ...
    def clean(self):
        if (
            None not in (self.related_name, self.to_id)
            and not self.to_model_class_is_mutable
        ):
...
to_field_help_text = _(
    "The field on the related object that the " "relation is to."
)
on_delete_help_text = _(
    "Behavior when an object referenced by this field " "is deleted"
)
...
class ForeignKeyDefinition(RelatedFieldDefinition):
    ...
    to_field = PythonIdentifierField(
        _("to field"), blank=True, null=True, help_text=to_field_help_text
    )
    one_to_one = fields.BooleanField(editable=False, default=False)
    on_delete = fields.CharField(
        _("on delete"),
        blank=True,
        null=True,
        max_length=11,
    ...
    on_delete_set_value = PickledObjectField(
        _("on delete set value"), null=True
    )
    ...
    def clean(self):
        ...
        if self.on_delete == self.ON_DELETE_SET_NULL:
            ...
            self.on_delete == self.ON_DELETE_SET_DEFAULT
            and self.default == fields.NOT_PROVIDED
        ):
            msg = _("This field has no default value")
            messages["on_delete"] = [msg]
        elif (
            self.on_delete == self.ON_DELETE_SET_VALUE
            and self.on_delete_set_value is None
        ):
...
class ManyToManyFieldDefinition(RelatedFieldDefinition):
    ...
    db_table = fields.SlugField(
        max_length=30, blank=True, null=True, help_text=db_table_help_text
    )
    ...
    def clean(self):
        ...
        if self.symmetrical is not None and not self.is_recursive_relationship:
            msg = _(
                "The relationship can only be symmetrical or not if it's "
        ...
        if self.through:
            if self.db_table:
                msg = _(
                    "Cannot specify a db_table if an intermediate "
                    "model is used."
                )
            ...
            if self.symmetrical:
                msg = _(
                    "Many-to-many fields with intermediate model cannot "
                    "be symmetrical."
                )
            ...
            if self.is_recursive_relationship():
                if seen_from > 2:
                    msg = _(
                        "Intermediary model %s has more than two foreign "
                        "keys to %s, which is ambiguous and is not permitted."
                    )
                    formated_msg = msg % (
                        through_class._meta.object_name,
                        from_model._meta.object_name,
                    )
                    messages.setdefault("through", []).append(formated_msg)
            else:
                msg = _(
                    "Intermediary model %s has more than one foreign key "
